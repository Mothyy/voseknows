...
import { AmexScraper } from '../scrapers/AmexScraper';
import { WestpacScraper } from '../scrapers/WestpacScraper';
...
export class ScraperService {
    static getScraper(slug: string, config: ScraperConfig): BankScraper {
        switch (slug.toLowerCase()) {
            ...
            case 'amex':
                return new AmexScraper(config);
            case 'wbc':
            case 'westpac':
                return new WestpacScraper(config);
            default:
